const myServices = {
  ...
  create: async (model, data) => {
    try {
      ...
      return {
        success: true,
        data: newRecord,
        message: `New ${model.name} created successfully!`,
      };
    } catch (error) {
      console.log('error is', error);
      return {
        success: false,
        message: `Error creating ${model.name}: ${error.message}`,
      };
    }
  },
  ...
  read: async (model, id, include = null, where = {}) => {
    try {
      // Dynamically get the primary key from the model
      const primaryKeyField = Object.keys(model.primaryKeys)[0] || "id";

      const options = {
        where: { ...where, [primaryKeyField]: id },
      };
      ...
      if (!record) {
        return {
          success: false,
          message: `${model.name} with ${primaryKeyField} ${id} not found`,
        };
      }
      ...
      return {
        success: true,
        data: record,
        message: `${model.name} with ${primaryKeyField} ${id} retrieved successfully!`,
      };
    } catch (error) {
      return {
        success: false,
        message: `Error retrieving ${model.name} with id ${id}: ${error.message}`,
      };
    }
  },
  ...
  update: async (model, id, data) => {
    try {
      ...
      return {
        success: true,
        data: updatedRecord,
        message: `${model.name} with id ${id} updated successfully!`,
      };
    } catch (error) {
      return {
        success: false,
        message: `Error updating ${model.name} with id ${id}: ${error.message}`,
      };
    }
  },
  ...
  delete: async (model, id) => {
    try {
      ...
      return {
        success: true,
        message: `${model.name} with id ${id} deleted successfully!`,
      };
    } catch (error) {
      return {
        success: false,
        message: `Error deleting ${model.name} with id ${id}: ${error.message}`,
      };
    }
  },
  ...
  list: async (model, include = null, where = {}, limit = 10, offset = 0, order = [['createdAt', 'DESC']]) => {
    try {
      ...
      return {
        success: true,
        data: records,
        message: `All ${model.name}s retrieved successfully!`,
      };
    } catch (error) {
      return {
        success: false,
        message: `Error retrieving ${model.name}s: ${error.message}`,
      };
    }
  },
  ...
  listPagination: async (
    ...
    try {
      ...
      return {
        success: false,
        message: `Error fetching paginated ${model.name}s: ${error.message}`,
      };
    }
  },
  ...
  checkExist: async (model, where = {}) => {
    try {
      ...
      if (!record) {
        return {
          success: false,
          message: `No ${model.name} record found matching the criteria.`,
        };
      }
  ...
  updateByWhere: async (model, where, data) => {
    try {
      ...
      if (!record) {
        return {
          success: false,
          message: `${model.name} not found with the provided criteria`,
        };
      }
